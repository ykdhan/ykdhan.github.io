...
const scss = () => {
	...
	return gulp.src([basePath.src + src.scss])
...
const scripts = ()=> {
  const isProduction = !!(_env.isProduction === true);
  return gulp.src([basePath.src + src.scripts])
    .pipe($.plumber({errorHandler: onError}))
...
const images = ()=> {
	...
	return gulp.src([ basePath.src + src.images ])
...
const favicons = ()=> {
	...
	return gulp.src([ basePath.src + src.favicons ])
		.pipe($.newer({dest: basePathImages}))
		.pipe($.print.default())
...
const models = ()=> {
    return gulp.src([ basePath.src + src.models ])
      .pipe($.print.default())
      .pipe(gulp.dest( basePath.dest + paths.models ))
}

const fonts = ()=> {
    return gulp.src([ basePath.src + src.fonts ])
      .pipe($.print.default())
      .pipe(gulp.dest( basePath.dest + paths.fonts ))
}

const html = ()=> {
  	return gulp.src([ basePath.src + src.html, basePath.src + src.ejs ])
		.pipe($.plumber({errorHandler: onError}))
		.pipe($.data((file) => {
...
const watchFiles = (cb) => {
	...
	const watcher = gulp.watch([ basePath.src + src.images ]);
...
const dev = gulp.parallel(models, fonts, favicons, images, scripts, scss, html);
const build = gulp.series(clean, setProduction, dev);
const defaults = gulp.series(dev, serve, watchFiles);
export { dev, build, clean, scss, scripts, images, html, models }
